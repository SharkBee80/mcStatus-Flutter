...
class FileUtils {
  ...
  /// 获取特定平台下的应用数据目录
  static Future<Directory> getApplicationDataDirectory() async {
    if (kIsWeb) {
      // Web平台不支持应用数据目录
  ...
  /// 获取各平台的标准配置文件路径
  static Future<Directory> getStandardConfigPath([String? fileName]) async {
    final directory = await getApplicationDataDirectory();
    DebugX.console("filepath: $directory");
    if (fileName == null) {
      return directory;
    } else {
      var path = Directory('${directory.path}/$fileName');
      return path;
    }
  }
}
